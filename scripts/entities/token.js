 ...
 */
export class TokenDocumentQueteRapide extends TokenDocument {
    /** @inheritdoc */
    getBarAttribute(...args) {
        const data = super.getBarAttribute(...args);
        if ( data && (data.attribute === "resolve") ) {
            data.value += parseInt(getProperty(this.actor.data, "data.resolve.temp") || 0);
            data.max += parseInt(getProperty(this.actor.data, "data.resolve.tempmax") || 0);
        }
        return data;
    }
    ...
    static getTrackedAttributes(data, _path=[]) {
        ...
        // Change to config quete rapide
        const allowed = [
            "attributes.resolve.value"
          ];
 ...
 * @extends {Token}
 */
 export class TokenQueteRapide extends Token {
    /** @inheritdoc */
    _drawBar(number, bar, data) {
      if ( data.attribute === "resolve" ) return this._drawHPBar(number, bar, data);
    ...
    _drawHPBar(number, bar, data) {
        // Extract health data
        let {value, max, temp, tempmax} = this.document.actor.data.data.resolve;
        temp = Number(temp || 0);
